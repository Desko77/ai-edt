...
def resource_text(found: dict[str, dict[str, object]]) -> str:
    ...
    rows = ["# derived by scripts/check-operation-params.py - do not edit",
            ...
            "# column 5 is read by the facade before it dispatches, so it is the same for every",
            "# operation the facade has. Both are parameters a caller may send; only the first is",
            "# established as this operation's, which is what per-operation help may print."]
    declared = kinds_by_class()
    agreed = kind_everywhere(declared)
    for _, row in sorted(found.items()):

...
public class EditFormTool implements IMcpTool
{
        ...
        throws Exception
    {
        ...
        // A data-bound field with no explicit title is left untitled on purpose:
        // the platform then shows the synonym of the attribute it binds to, which
        // is what the Designer produces and what a user expects to read in a
        // column header. Writing the element name here instead - the old
        // behaviour - put the technical name on screen and could only be undone
        // by deleting the <title> block by hand. Unbound fields keep the name as
        // a title, since they have no attribute to inherit one from.
        ...
        if (isTableColumn)
        {
            helper.applyTableColumnDefaults(field, fieldType, hyperlink);
        }

        // The hyperlink flag reaches the element two different ways. A standalone
        // field carries it in LabelFieldExtInfo (the sole field ext-info exposing
        // setHyperlink), a column carries it in the FormField cell properties -
        // and a column is a hyperlink whatever its field kind. Outside a table a
        // non-Label field cannot be one, so say so rather than report success.
        boolean isLabelField = fieldType.toLowerCase().contains("label"); //$NON-NLS-1$
        String hyperlinkLine = !hyperlink ? "" //$NON-NLS-1$
            : (isTableColumn ? "- Hyperlink: true (cell hyperlink: readOnly, editMode=EnterOnInput)\n" //$NON-NLS-1$
                : (isLabelField ? "- Hyperlink: true\n" //$NON-NLS-1$
                    : "- Hyperlink: ignored (outside a table only Label fields can be hyperlinks)\n")); //$NON-NLS-1$

        return buildSuccess("edit_form", name, "add_field", //$NON-NLS-1$ //$NON-NLS-2$
            "Field '" + name + "' added to form successfully.\n" + //$NON-NLS-1$ //$NON-NLS-2$
            "- Name: " + name + "\n" + //$NON-NLS-1$ //$NON-NLS-2$
            "- Title: " + (title == null || title.isEmpty() //$NON-NLS-1$
                ? "(inherited from the bound attribute)" : title) + "\n" + //$NON-NLS-1$ //$NON-NLS-2$
        ...
        java.util.List<String> warnings)
    {
        try
        {
            ...
            for (ColumnSpec col : columns)
            {
                ...
                helper.applyTableColumnDefaults(field, fieldType, false);

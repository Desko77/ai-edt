...
public class ProjectAdminFacadeTool implements IMcpTool
{
    ...
    private static Map<String, Supplier<IMcpTool>> buildDescribed()
    {
        ...
        m.put("list_projects", ProjectsLister::new); //$NON-NLS-1$
        m.put("list_configurations", LaunchConfigsLister::new); //$NON-NLS-1$
        m.put("get_configuration_properties", ConfigurationInfoReader::new); //$NON-NLS-1$
    ...
    private static String buildHelp(String topic, String find, String schema)
    {
        ...
        return FacadeParameterHelp.answer(topic, DESCRIBED, OPS.keySet(),
            "workflow", "ProjectAdminFacadeTool", schema, buildHelp(null, null, schema)); //$NON-NLS-1$
    }

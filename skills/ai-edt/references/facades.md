...
**An ordinary form's module lives in its container.** There is no `Module.bsl` beside a `Form.oform`; the module tools resolve the address `<type>/<object>/Forms/<form>/Module.bsl` (or the FQN `Type.Object.Form.Name`) to the container and read the module entry out of it. The answer carries `source: oform` and `container`. `write_module_source` writes the module entry back with the layout entry untouched, answers `persistenceSyncDetail: not applicable` (EDT holds no model to flush) and no validation (the platform checks the module at `update_database`). `handlerChanges=warn|refuse` decides what happens when the write removes a procedure the layout binds an event to (`{3,"Name"` records) or one the module names in a string literal: `warn` writes and lists them under `unboundProcedures`, `refuse` leaves the container unchanged. The answer's `delivery` line says whether the form's owner now reads as changed in the project's infobase baselines: EDT's incremental update does not map a changed `Form.oform` to an object, so the writer marks the owner, and the next `update_database` loads the object with the form (`Ext/Form.bin`, the container byte for byte). `list_modules` shows the kind `OrdinaryFormModule`; `code_search operation=text_search` scans the containers and reports hits under the module address; `call_hierarchy`, `object_references` and `get_project_errors` end with a `Coverage` line over a project that has such forms.

...
| A data-path refusal on an extension form, naming the element, the path and a reason | The path would not be exported with the extension form - the attribute it starts at is not borrowed, or EDT drops the path itself. Nothing was written. | Read the reason. An unborrowed attribute: borrow it with `extension_workshop operation=borrow_object`. A dropped path: bind to an attribute the extension can carry, or drop the path. Repeating the same call cannot succeed. |

...
[![1C:EDT](https://img.shields.io/badge/1C%3AEDT-2026.1.2_%7C_2026.2.0-58a6ff?style=flat-square)](#-1-требования)
...
> [!IMPORTANT]
> Текущая версия работает на **1C:EDT 2026.1.2** и **2026.2.0** и требует **Java 17**. Поддерживаемые версии перечисляются поименно - полный список и почему более ранние не подойдут в разделе [Требования](#-1-требования). Сборка идет против 2026.1, поэтому один артефакт ставится на обе. Плагин работает везде, где работает EDT; на Windows рассчитаны только описанные ниже скрипты сборки и установки.
...
- 1C:EDT - конкретные версии, см. список ниже
- Java / JDK **17**
...
#### Поддерживаемые версии 1C:EDT

- **2026.1.2**
- **2026.2.0**

Версии перечислены поименно, нижней границы у списка нет. Запись «2026.1 и новее» была бы обещанием за
версии, которых еще нет, и за более ранние сборки той же линейки, которые никто не проверял. Версия
попадает в список только после того, как совместимость обеспечена и проверена: указана - значит
проверена. Все, чего в списке нет, считается неподдерживаемым, даже если формально новее перечисленного.

Более ранние сборки линейки 2026.1 (включая 2026.1.0) не проверялись. Плагин требует
`dt.metadata.common` 4.0, `dt.metadata.mdclass` 12.0 и `dt.metadata.mdclass.util` 6.0, а в текущей 2026.1
эти пакеты имеют ровно такие версии - запас нулевой, поэтому более ранняя сборка линейки может не
подойти.

На 2025.2 и более ранних плагин не установится. Те же три пакета там ниже требуемого
(`dt.metadata.common` 3.14, `dt.metadata.mdclass` 11.0, `dt.metadata.mdclass.util` 5.18), а на 2025.1
дополнительно отсутствует `dt.platform.services.core.infobases.sync.v2`. Установщик p2 сообщает об этом
как «Missing requirement».

...
Каждый запрос к серверу несет bearer-токен. Токен создается при первом запуске сервера и показан в
**Window → Preferences → AI-EDT**, поле **Bearer token** - скопируйте его в конфигурацию клиента.
Клиент без токена получает `401`.
...
Сервер слушает только `127.0.0.1`, и каждый запрос к `/mcp` несет bearer-токен.

- **Bearer-токен обязателен.** Он создается при первом запуске сервера, если поле на странице **Preferences → AI-EDT** пусто, и показан только там. Клиент передает его заголовком `Authorization: Bearer <токен>`; сравнение идет за константное время. Запрос без верного токена получает `401`, тело отказа называет страницу настроек и не содержит токена. В журнал рабочей области токен не пишется. Кнопка **Generate** подставляет новый токен, **Apply** с пустым полем создает его сам; новый токен действует после того, как сохранен в настройках, - пока запись не завершена, действует прежний.
- **`/health` без токена** отвечает тремя полями: `status`, `phase`, `edt_version`. Имя рабочей области, текущий инструмент, показатели кучи и очереди - только с токеном.
...
- Endpoint слушает loopback, и каждый запрос несет bearer-токен со страницы настроек. Привязка ко всем интерфейсам включается отдельно - см. раздел про безопасность.

...
Готовый плагин публикуется как update site, поэтому собирать его самостоятельно не обязательно:

```text
https://desko77.github.io/ai-edt/
```

Этот адрес указывается в EDT так же, как локальный архив. Дальше описаны оба пути - через интерфейс EDT и из командной строки; ставят они одну и ту же фичу.

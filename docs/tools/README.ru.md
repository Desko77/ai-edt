...
| `git` | Фасад · Чтение и запись | Git репозитория проекта внутри IDE: `status` (дерево и индекс против HEAD, ahead/behind по tracking-ветке), `branches`, `log` (последние коммиты, `limit` до 100), `commit` (постановка и коммит названных файлов - `paths` поименно и обязательно, add-all отвергается; автор из конфигурации репозитория или `authorName`/`authorEmail`; в ответе `boundInfobase`, когда ветка привязана к ИБ). Запись (`commit`) отвергается под пресетом Read-only. |
| `git_commit` | Запись (операция фасада `git`) | Имя, под которым операция `commit` гасится пресетами: под Read-only вызов отвергается до постановки файлов. Отдельным инструментом не является. |
| `git_checkout` | Запись (операция фасада `git`, в работе) | Имя, зарезервированное под гейт операции `checkout` (переход по ветке / создание ветки) для тех же пресетов. Отдельным инструментом не является. |

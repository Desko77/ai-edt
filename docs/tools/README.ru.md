...
| `git` | Фасад · Чтение | Git репозитория проекта внутри IDE: `status` (дерево и индекс против HEAD, ahead/behind по tracking-ветке), `branches`, `log` (последние коммиты, `limit` до 100). Только чтение - ничего не меняет в дереве, индексе или истории. |

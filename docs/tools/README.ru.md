...
| `debug_launch` | Выполнение | Запускает клиент или подключается к серверу отладки 1С. С `externalObjectName` клиент, запущенный под отладчиком, открывает названную внешнюю обработку или отчет (объект берется из проекта внешних объектов; `externalObjectProject` называет его, когда это не тот же проект). Имя, которое не разрешается, останавливает запуск ДО обновления базы. |

...
| `compare_three_way` | Чтение и запись | Сравнивает открытый проект с новой поставкой и с поставкой, от которой обе произошли. По умолчанию читает. При `intent=MERGE` применяет принятые решения к проекту - необратимо; под пресетом без права записи отклоняется. |

...
> [!NOTE]
> Это первая статическая версия справочника. Названия и группировка сверены с `ToolCategory` и реестром `McpHttpEndpoint.declareAllTools()`. Подробные JSON-схемы параметров пока остаются источником истины в `tools/list`; в дальнейшем каталог целесообразно генерировать автоматически.
...
- [Без назначенной группы](#без-назначенной-группы) - зарегистрированы, но отсутствуют в `ToolCategory`
